...
class TTSProsody(object):
    def __init__(self, path, device):
        ...
        state_dict = torch.hub.load_state_dict_from_url(
            "https://github.com/PlayVoice/vits_chinese/releases/download/v1.0/prosody_model.pt",
            map_location="cpu",
        )
        self.char_model.load_state_dict(state_dict, strict=False)
    ...
    def get_char_embeds(self, text):
        ...
        with torch.no_grad():
            char_embeds = self.char_model(input_ids, input_masks, type_ids).squeeze(0).cpu()
        return char_embeds
